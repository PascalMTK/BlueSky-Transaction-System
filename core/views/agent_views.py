...
def _send_transaction_email(tx, client_email: str, locale: str = 'fr') -> bool:
    """Send a styled HTML confirmation email to the client using the configured SMTP."""
    ...
    if is_en:
        ...
        lbl_base      = "Amount"
        lbl_fee       = "Fees"
        lbl_total     = "Total paid"
        type_val      = "Transfer" if is_send else "Withdrawal"
        thanks        = "Thank you for choosing <b>BLUESKY Transactions</b> for your money transfers."
        footer_note   = "Automated notification — please do not reply."
        lbl_summary   = "TRANSACTION SUMMARY"
        lbl_financial = "FINANCIAL DETAILS"
    else:
        ...
        lbl_base      = "Montant"
        lbl_fee       = "Frais"
        lbl_total     = "Total payé"
        type_val      = "Transfert" if is_send else "Retrait"
        thanks        = "Merci de faire confiance à <b>BLUESKY Transactions</b> pour vos transferts d'argent."
        footer_note   = "Notification automatique — merci de ne pas répondre à cet email."
        lbl_summary   = "RÉSUMÉ DE LA TRANSACTION"
        lbl_financial = "DÉTAILS FINANCIERS"
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
        ...
        <tr><td style="background:linear-gradient(145deg,#002d6e 0%,#0055b3 50%,#0096d6 100%);border-radius:16px 16px 0 0;padding:32px 32px 24px;text-align:center;">
          ...
          <div style="margin-top:18px;">
            <span style="display:inline-block;background:rgba(255,255,255,.18);border:1px solid rgba(255,255,255,.3);border-radius:50px;padding:7px 20px;color:#fff;font-size:13px;font-weight:700;">&#10003; {subject.replace('✅ ','').split(' — ')[0]}</span>
          </div>
        </td></tr>
        ...
        <tr><td style="background:#ffffff;padding:0 32px 20px;">
          <div style="background:#f0f7ff;border-left:4px solid #0284c7;border-radius:0 8px 8px 0;padding:12px 16px;display:flex;align-items:center;">
            <span style="color:#64748b;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:.6px;">{lbl_ref} &nbsp;</span>
            <span style="color:#0284c7;font-size:14px;font-weight:900;font-family:monospace;letter-spacing:.5px;">{tx.transaction_number}</span>
          </div>
        </td></tr>
        ...
        <tr><td style="background:#ffffff;padding:0 32px 20px;">
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e8edf2;border-radius:10px;overflow:hidden;font-size:13.5px;">
            <tr style="border-bottom:1px solid #e8edf2;">
              <td style="padding:11px 14px;color:#64748b;font-weight:600;width:45%;">{lbl_type}</td>
              <td style="padding:11px 14px;color:#0f172a;font-weight:700;text-align:right;">{type_val}</td>
            </tr>
            <tr style="border-bottom:1px solid #e8edf2;background:#fafbfc;">
              <td style="padding:11px 14px;color:#64748b;font-weight:600;">{lbl_origin}</td>
              <td style="padding:11px 14px;color:#0f172a;text-align:right;">{origin_flag} {origin_name}</td>
            </tr>
            <tr style="border-bottom:1px solid #e8edf2;">
              <td style="padding:11px 14px;color:#64748b;font-weight:600;">{lbl_dest}</td>
              <td style="padding:11px 14px;color:#0f172a;text-align:right;">{dest_flag} {dest_name}</td>
            </tr>
            <tr style="border-bottom:1px solid #e8edf2;background:#fafbfc;">
              <td style="padding:11px 14px;color:#64748b;font-weight:600;">{lbl_date}</td>
              <td style="padding:11px 14px;color:#0f172a;text-align:right;">{date_str}</td>
            </tr>
            <tr>
              <td style="padding:11px 14px;color:#64748b;font-weight:600;">{lbl_agent}</td>
              <td style="padding:11px 14px;color:#0f172a;text-align:right;">{agent_name}</td>
            </tr>
          </table>
        </td></tr>
        ...
        <tr><td style="background:#ffffff;padding:0 32px 28px;">
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="font-size:13.5px;">
            <tr>
              <td style="padding:6px 0;color:#64748b;">{lbl_base}</td>
              <td style="padding:6px 0;color:#0f172a;font-weight:600;text-align:right;">{base_str}</td>
            </tr>
            <tr>
              <td style="padding:6px 0;color:#64748b;">{lbl_fee} ({tx.fee_percentage:.1f}%)</td>
              <td style="padding:6px 0;color:#0f172a;font-weight:600;text-align:right;">{fee_str}</td>
            </tr>
          </table>
          <!-- Total highlight -->
          <div style="margin-top:12px;background:linear-gradient(135deg,#0055b3,#0096d6);border-radius:10px;padding:14px 18px;">
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td style="color:rgba(255,255,255,.8);font-size:13px;font-weight:700;">{lbl_total}</td>
                <td style="color:#ffffff;font-size:22px;font-weight:900;text-align:right;">{amount_str}</td>
              </tr>
            </table>
          </div>
        </td></tr>
        ...
        <tr><td style="background:#f1f5f9;border-radius:0 0 16px 16px;padding:16px 32px;text-align:center;border-top:1px solid #e2e8f0;">
          <p style="margin:0;color:#94a3b8;font-size:11px;line-height:1.6;">{footer_note}</p>
          <p style="margin:6px 0 0;color:#cbd5e1;font-size:10.5px;">© {year} BLUESKY Transactions — All rights reserved</p>
        </td></tr>
